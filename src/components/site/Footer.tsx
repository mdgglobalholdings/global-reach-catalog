    ...
    <footer style={{ backgroundColor: "#021807" }} className="text-white">
        ...
        <div className="grid gap-8 grid-cols-2 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="col-span-2 sm:col-span-2 lg:col-span-1">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-display text-xs sm:text-sm text-white tracking-wider">MDG GLOBAL HOLDINGS</div>
                <div className="mt-0.5 text-or-prestige font-bold text-[10px] sm:text-xs">{COMPANY.tagline}</div>
              </div>
            </div>
            <p className="mt-4 text-xs sm:text-sm text-white/70 leading-relaxed max-w-[32ch]">
              Votre partenaire pour entreprendre, acheter et développer au-delà des frontières.
            </p>
            <div className="mt-3 font-mono text-[9px] sm:text-[10px] uppercase tracking-[0.12em] text-white/40">
          ...
          {/* Navigation */}
          <div>
            <h3 className="text-sm sm:text-base font-bold uppercase tracking-wider text-white mb-4 sm:mb-6">Navigation</h3>
            <nav className="space-y-2 sm:space-y-3">
              {[
                ...
                <Link key={item.label} to={item.to}
                  className="block text-xs sm:text-sm text-white/70 smooth-transition hover:text-or-lumiere">
                  {item.label}
                </Link>
              ))}
            </nav>
          ...
          {/* Contact */}
          <div>
            <h3 className="text-sm sm:text-base font-bold uppercase tracking-wider text-white mb-4 sm:mb-6">Contact</h3>
            <dl className="space-y-3 text-xs sm:text-sm">
              ...
              <div>
                <dt className="font-bold text-or-prestige text-[9px] sm:text-[10px] uppercase tracking-wider mb-0.5">Adresse</dt>
      ...
      <div className="border-t border-or-prestige/20">
        <div className="mx-auto max-w-[1280px] px-4 sm:px-6 py-4 flex flex-col sm:flex-row gap-2 sm:justify-between sm:items-center">
          <div className="text-[10px] sm:text-sm text-white/50">© {new Date().getFullYear()} MDG GLOBAL HOLDINGS · Tous droits réservés</div>
