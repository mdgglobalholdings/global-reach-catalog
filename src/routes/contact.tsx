...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      { title: "Contact & Quote — MDG GLOBAL HOLDINGS | Response within 24h" },
      { name: "description", content: "Contact MDG GLOBAL HOLDINGS in Ouagadougou: WhatsApp, phone, email. Personalised quote within 24h for vehicles, logistics, international sourcing and visa assistance." },
      { property: "og:title", content: "Contact — MDG GLOBAL HOLDINGS" },
      { property: "og:description", content: "Contact MDG GLOBAL HOLDINGS for a personalised quote within 24 hours." },
    ],
  }),
    ...
    <main>
      ...
      <section className="relative overflow-hidden" style={{ backgroundColor: "#eeeee6" }}>
        ...
        <div className="mx-auto max-w-[1280px] px-4 sm:px-6 py-12 sm:py-16">
          <p className="text-base sm:text-lg font-bold uppercase tracking-widest text-or-prestige">{c.label}</p>
        ...
        <div className="mx-auto max-w-[1280px] px-4 sm:px-6">
          <div className="grid gap-10 lg:grid-cols-[1fr_1.4fr] min-w-0">
            ...
            <div className="min-w-0">
              ...
              <h2 className="mt-6 text-2xl uppercase">{c.coordTitle}</h2>
              <p className="mt-3 text-sm text-noir/65">{c.coordHours}</p>

              <dl className="mt-8 space-y-5">
                {[
                  { label: c.labelBf, value: COMPANY.phoneBf, href: telLink(COMPANY.phoneBf) },
                  { label: c.labelCn, value: COMPANY.phoneCn, href: telLink(COMPANY.phoneCn) },
                  { label: c.labelEmail, value: COMPANY.email, href: `mailto:${COMPANY.email}` },
                  { label: c.labelWeb, value: COMPANY.website, href: `https://${COMPANY.website}` },
                ].map((item) => (
                  <div key={item.label}>
                    <dt className="label-mono text-or-prestige/60">{item.label}</dt>
                    <dd className="mt-1">
                      <a href={item.href} className="text-foreground hover:text-or-prestige smooth-transition text-sm font-medium">{item.value}</a>
                    </dd>
                  </div>
                ))}
                <div>
                  <dt className="label-mono text-or-prestige/60">{c.labelAddress}</dt>
              ...
              <div className="mt-8 space-y-3">
                <a
                  href={whatsappLink(lang === "en" ? "Hello MDG GLOBAL HOLDINGS, I would like a quote." : "Bonjour MDG GLOBAL HOLDINGS, je souhaite un devis.")}
            ...
            <div className="min-w-0 overflow-hidden">
              <div className="gold-line" />
              <h2 className="mt-6 text-2xl uppercase">{c.quoteTitle}</h2>
              <p className="mt-2 text-sm text-noir/65">{c.quoteText}</p>
              <div className="mt-6">
